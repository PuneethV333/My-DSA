 ...
 *
 */
SinglyLinkedListNode* insert_head(SinglyLinkedListNode* head, int data) {
...
SinglyLinkedListNode* reverse(SinglyLinkedListNode* head) {
    ...
    int* dataArr = malloc(count * sizeof(int));
    int pos = 0;
    SinglyLinkedListNode* pres = head;
    while (pres != NULL) {
        dataArr[pos++] = pres->data;
        pres = pres->next;
    }
    ...
    for (int i = count - 1; i >= 0; i--) {
        new_head = insert_head(new_head, dataArr[count-i-1]);
    }
